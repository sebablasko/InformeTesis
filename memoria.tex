\documentclass[upright, contnum]{umemoria}
\depto{DEPARTAMENTO DE CIENCIAS DE LA COMPUTACIÓN}
\author{SEBASTIÁN RAMÓN BLASCO VALENCIA}
\title{Estudio de detección y amortización de contención sobre la interfaz de red en sistemas Linux en escenarios de concurrencia sobre máquinas multicore}
\auspicio{NIC CHILE RESEARCH LABS}
\date{2016}
\guia{Javier Bustos Jiménez}
\carrera{MAGÍSTER EN CIENCIAS, MENCIÓN COMPUTACIÓN}
\memoria{TESIS PARA OPTAR AL GRADO DE}
\carreraing{INGENIERO CIVIL EN COMPUTACIÓN}
\memoriaing{MEMORIA PARA OPTAR AL TITULO DE}
\comision{Luis Mateu Brulé}{Nancy Hitschfeld Kahler}{Carolina Bonacic Castro}

\usepackage{lipsum}
\usepackage{subfigure}
\usepackage{bigfoot}
\usepackage{tikz}
\usepackage{amsmath}
\newcommand{\timeline}{\hspace{-2.3pt}$\bullet$ \hspace{5pt}}

\usepackage{minted}
\usepackage{multirow}
\colorlet{LightGray}{gray!5!}
\renewcommand{\lstlistingname}{Código}
\definecolor{mygray}{rgb}{0.95,0.95,0.95}
\definecolor{mygray2}{rgb}{0.99,0.99,0.79}

%Defino un par de estilos
\lstdefinestyle{CInputStyle}{
  language=C,
  basicstyle=\small\sffamily,
  numbers=left,
  numberstyle=\tiny,
  numbersep=3pt,
  frame=tb,
  columns=fullflexible,
  backgroundcolor=\color{mygray},
  linewidth=0.9\linewidth,
  xleftmargin=0.05\linewidth
}

\lstdefinestyle{BashInputStyle}{
  language=bash,
  basicstyle=\small\sffamily,
  frame=tb,
  columns=fullflexible,
  backgroundcolor=\color{mygray2}
}

\begin{document}

\frontmatter
\maketitle

\input{abstract.tex}

\begin{dedicatoria} % opcional
%Una dedicatoria corta. Por ejemplo, \emph{A los creadores de U-Campus}
A la Yaya, que me acompaña siempre.
\end{dedicatoria}

\input{thanks.tex}
\cleardoublepage

\tableofcontents
\listoftables % opcional
\listoffigures % opcional

\mainmatter

%\input{intro.tex}
\input{intro/intro.tex}
%\input{cap1.tex}
%\input{cap2.tex}
\input{cap_validacionProblema/capitulo.tex}
%\input{capitulo2/capitulo.tex}
\input{cap_estudioLlamadasSistema/capitulo.tex}
\input{cap_estudioPerformanceCounters/capitulo.tex}
\input{cap_estudioProcessorAffinity/capitulo.tex}
%\input{capitulo3/capitulo.tex}
\input{cap_reuseport/capitulo.tex}
%\input{capitulo4/capitulo.tex}
\input{cap_solucion/capitulo.tex}
\input{conclu.tex}

% \input{glosario.tex} % opcional

\bibliographystyle{plain}
\bibliography{bibliografia}

% \input{anexo_apendices.tex} % opcionales

\end{document}

%%% Local Variables:
%%% mode: latex
%%% TeX-master: t
%%% End:
