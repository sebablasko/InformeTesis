...
\begin{figure}[!h]
	\centering
	\includegraphics[scale=.5]{imagenes/perfRecord.png}
...
\begin{defn}[ver \cite{paper:cachebouncing}] \textbf{Caché Bouncing} corresponde a un fenómeno producido en entornos multiprocesador, cuando distintas CPU realizan modificaciones a una linea de caché especifica que está siendo referenciada por varios procesadores. La modificación de la linea modificada se trasnfiere de caché en caché según los protocolos de consistencia del sistema. Éste fenómeno impone una significativa carga en el bus de memoria y los canales de comunicación afectados.
\end{defn}
...
\begin{figure}[!h]
	\centering
	\includegraphics[scale=.65]{imagenes/arch24Cores.png}
...
\begin{lstlisting}[language=Bash, breaklines=true, frame=single, captionpos=b, caption=Ejemplo de uso de Perf para colectar datos de una colección de eventos sobre un script llamado programa. En éste caso se configura para colectar datos de 2 eventos y dejar el reporte de salida en un archivo resultado.txt]
