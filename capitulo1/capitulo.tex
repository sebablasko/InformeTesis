...
Corresponden a un conjunto de instrucciones provistas por hardware, disponibles en ambientes multi-procesador, que brindan la flexibilidad de que aún siendo ejecutadas simultaneamente en distintas CPU's, en la práctica son ejecutadas secuencialmente en un orden arbitrario. La exclusión mutua es provista en éste caso usando variables de naturaleza \verb=Boolean= que se denominan \emph{lock}. Existen diversas implementaciones de operaciones de éste tipo, siendo las más conocidas: \emph{Test-and-Set}, que valida una condición y si se cumple realiza una asignación, \emph{Swap}, que intercabia el valor contenido entre dos variables, etc. Todas atómicas.
...
\begin{figure}[h!]
	...
	\hspace*{\fill}
	\subfigure[]{
		\includegraphics[width=.47\textwidth]{resultados/kernel1}
		\label{fig:tests_kernel1}
	}\hfill
	\subfigure[]{
		\includegraphics[width=.47\textwidth]{resultados/kernel2}
