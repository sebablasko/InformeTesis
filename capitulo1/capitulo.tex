...
Para ello, se diseñó una prueba para evaluar el tiempo de consumo promedio de datos desde una interfaz de red\footnote{URL al repo github con la prueba}, representada por un socket UDP en estado de saturación. El modelo de la prueba incluye la evaluación del tiempo de consumo de una cantidad de datos de entrada correspondiente a 1 millon de paquetes de 512 bytes cada uno, emulando de ésta manera un escenario clásico de congestión de un servidor de consultas DNS. Se evalúan distintas configuraciones de threads consumiendo concurrentemente al mismo socket rescatando el tiempo total de consumo. Para respetar la validez estadística de los resultados, la prueba anterior se repite un total de 60 veces por configuración de threads.
...
\begin{description}
\item[] 2, hace referencia a la imagen \ref{fig:pc2}
\item[PC3] 3, hace referencia a la imagen \ref{fig:pc3}
\item[PC1] 1, hace referencia a la imagen \ref{fig:pc1}
\end{description}

\begin{figure}[th!]
\centering
\subfigure[Pc2]{
	\includegraphics[width=.35\textwidth]{imagenes/pc2_1.pdf}
	\label{fig:pc2}
}
\subfigure[Pc3]{
	\includegraphics[width=.45\textwidth]{imagenes/fcfm}
	\label{fig:pc3}
}
\subfigure[Pc1]{
	\includegraphics[width=.75\textwidth]{imagenes/pc1_1.pdf}
	\label{fig:pc1}
}
...
Al igual que con la arista de hardware, el fenómeno de mal rendimiento de threads detectado podría ser propio de una determinada versión del kernel de Linux, una sospecha que se acentúa al recordar que parte importante del código fuente del núcleo es heredado entre versiones, y que las versiones más populares del kernel abarcan una importante razón de tiempo entre sus publicaciones. A modo de ilustración de esto último, a continuación se muestra una linea de tiempo con algunos de los releases más importantes del kernel de linux:

$$\scalebox{1.2}{
\begin{tabular}{r |@{\timeline} l}
4 Enero 2012 & 3.2\\
30 Junio 2013 & 3.10\\
2 Noviembre 2013 & 3.12\\
30 Marzo 2014 & 3.14\\
12 Abril 2015 & 4.0.4\\
\end{tabular}
}$$
