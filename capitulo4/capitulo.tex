...
El enfoque antes planteado es sumamente poderoso y extensible pues en la práctica postula la capacidad de multiplexar una misma interfaz de socket del sistema, proveyéndole recepción de paquetes a un pool de otros sockets atendidos por aplicaciones independientes. En otras palabras, se podría multiplezar una misma interfaz lógica del sistema, asociada a un puerto del sistema para atender una variedad de servicios, rigiendo la distribución según las características mismas de un paquete (Ver imagen \ref{fig:multiplexarPuerto}).

Tan flexible es éste enfoque que permite soportar una diversidad de servicios (ya sean orientados a la conexión o a la mensajería) sobre una única interfaz lógica de conexión según las capas 3 y 4 del modelo OSI. Ésto en la práctica ha de permitir mejoras en aspectos como seguridad pues sólo los servicios esperados consiguen realizar conexiones correctas. Un ejemplo práctico de llo anterior se ilustra en la imagen \ref{fig:multiplexarPuertoEjemplo} donde se muestran distintas aplicaciones que operan trabajando con paquetes de diversa naturaleza (tanto por sus características, como por sus orígenes) siendo procesados a través de una misma interfaz lógica.
